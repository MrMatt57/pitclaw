...
The schematic is generated by `generate_schematic.py`. This script computes exact pin positions and wire endpoints so all connections are guaranteed to align.

```bash
python hardware/generate_schematic.py
```

To modify the schematic layout, edit component positions in the script and re-run. The script validates net label pairs on output.
...
- All wire endpoints are computed from pin definitions -- every connection is verified
